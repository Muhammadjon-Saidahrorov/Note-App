...
class TrashDialog : BottomSheetDialogFragment() {
    private var clickAddNotesButtonListener: (() -> Unit)? = null
    private var clickDeleteButtonListener: (() -> Unit)? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.trash_dialog, container)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        view.findViewById<LinearLayoutCompat>(R.id.lineEdit).setOnClickListener {
            clickAddNotesButtonListener?.invoke()
            dismiss()
        }
    ...
    fun setClickAddNotesButtonListener(block: () -> Unit) {
        clickAddNotesButtonListener = block
    }
